...
const localScore = computed({
    ...
    set: (value) => emit('update:score', { trackId: props.track.id, score: value })
})
...
<template>
    <div class="flex items-center gap-4 py-2 px-2 rounded transition-colors group
                hover:bg-gray-50 dark:hover:bg-white/5"
        :class="{ 'opacity-60 bg-gray-50/50 dark:bg-white/5': track.isIgnored }">

        <div class="w-8 relative flex items-center justify-center">

            <span class="text-sm font-mono text-gray-400 transition-opacity duration-200 group-hover:opacity-0"
                :class="{ 'opacity-0': track.isIgnored }">
                {{ track.track_number }}
            </span>

            <button @click="handleToggleIgnore"
        ...
        <div class="w-32 md:w-40 flex justify-end transition-opacity duration-300"
            :class="{ 'opacity-50 pointer-events-none grayscale': track.isIgnored }">
